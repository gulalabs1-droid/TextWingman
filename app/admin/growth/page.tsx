  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <section className={panelClass()}>
        <div className="flex flex-col gap-1 border-b border-white/[0.07] p-5 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2"><DollarSign className="h-4 w-4 text-emerald-400" /><h2 className="text-sm font-bold text-white/85">Revenue Funnel Header</h2></div><p className="mt-1 text-xs text-white/35">External visitors -&gt; people with a reply -&gt; signups -&gt; paid users -&gt; MRR. People are deduplicated; MRR is current active/trialing revenue.</p></div><span className="text-[10px] text-white/25">{data.range.days}d view</span></div>
        <div className="grid grid-cols-2 gap-3 p-4 lg:grid-cols-5">
          ...
          <MetricCard label="People with a reply" value={displayNumber(current.replySuccesses)} sub={`${displayRate(data.revenue.rates.visitorToReply)} visitor -> reply`} color="text-fuchsia-300" />
