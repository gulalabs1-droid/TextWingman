  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">Content &amp; Growth</h1>
          <p className="text-sm text-white/50">Usage patterns and engagement</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Replies', value: data.totalReplies, icon: MessageSquare, bg: 'bg-purple-500/15', color: 'text-purple-400' },
          { label: 'Total Copies', value: data.totalCopies, icon: Copy, bg: 'bg-blue-500/15', color: 'text-blue-400' },
          { label: 'Copy Rate', value: `${data.copyRate}%`, icon: BarChart3, bg: 'bg-green-500/15', color: 'text-green-400' },
          { label: 'Hit Free Limit', value: data.hitLimitCount, icon: Zap, bg: 'bg-orange-500/15', color: 'text-orange-400' },
        ].map(c => (
          <Card key={c.label} className="border border-white/[0.08] bg-white/[0.03]">
            <CardContent className="p-4">
              ...
              <p className="text-2xl font-bold text-white">{typeof c.value === 'number' ? c.value.toLocaleString() : c.value}</p>
            </CardContent>
          </Card>
        ))}
      </div>
              ...
              <div className="space-y-2">
                {sortedContexts.slice(0, 10).map(([ctx, count]) => (
                  <div key={ctx} className="flex items-center gap-2">
                    <span className="text-xs text-white/60 w-20 shrink-0 truncate capitalize">{ctx}</span>
              ...
              <div className="space-y-2">
                {sortedTones.map(([tone, count]) => (
                  <div key={tone} className="flex items-center gap-2">
                    <span className="text-xs text-white/60 w-20 shrink-0 capitalize">{tone}</span>
                    <div className="flex-1 h-5 bg-white/[0.06] rounded overflow-hidden">
                      <div
      ...
      <Card className="border border-white/[0.08] bg-white/[0.03]">
        ...
        <CardContent>
          {data.v2Stats.total === 0 ? (
            <p className="text-white/30 text-sm text-center py-6">No V2 runs yet</p>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div className="p-3 bg-green-500/10 rounded-lg text-center">
                <p className="text-xs text-green-400 font-medium">Pass Rate</p>
                <p className="text-xl font-bold text-white">{data.v2Stats.passRate}%</p>
              </div>
              <div className="p-3 bg-blue-500/10 rounded-lg text-center">
                <p className="text-xs text-blue-400 font-medium">Avg Confidence</p>
                <p className="text-xl font-bold text-white">{data.v2Stats.avgConfidence}%</p>
              </div>
              <div className="p-3 bg-orange-500/10 rounded-lg text-center">
                <p className="text-xs text-orange-400 font-medium">Avg Latency</p>
                <p className="text-xl font-bold text-white">{data.v2Stats.avgLatency}ms</p>
              </div>
              <div className="p-3 bg-purple-500/10 rounded-lg text-center">
                <p className="text-xs text-purple-400 font-medium">Avg Revisions</p>
                <p className="text-xl font-bold text-white">{data.v2Stats.avgReviseAttempts}</p>
              </div>
            </div>
          )}
        </CardContent>
