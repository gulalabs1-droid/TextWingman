...
export default function UsersPage() {
  ...
  const planBadge = (plan: string, source: string) => {
    ...
    return (
      <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${colors[plan] || 'bg-gray-100 text-gray-600'}`}>
        {plan}
    ...
    <div className="space-y-6">
      ...
      <Card className="border border-gray-200 overflow-hidden">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                ...
                <th className="text-right px-4 py-3 font-medium text-gray-500">7d</th>
                <th className="text-right px-4 py-3 font-medium text-gray-500">30d</th>
                <th className="text-right px-4 py-3 font-medium text-gray-500">Total</th>
                <th className="text-left px-4 py-3 font-medium text-gray-500">Last Reply</th>
            ...
            <tbody>
              {loading ? (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-right font-mono text-gray-700">{u.replies_7d}</td>
                  <td className="px-4 py-3 text-right font-mono text-gray-700">{u.replies_30d}</td>
                  <td className="px-4 py-3 text-right font-mono text-gray-700">{u.replies_lifetime}</td>
                  <td className="px-4 py-3 text-gray-500 text-xs">{timeAgo(u.last_reply_at)}</td>
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200 bg-gray-50">
          <p className="text-xs text-gray-500">Page {page} of {Math.ceil(total / 50) || 1}</p>
          <div className="flex gap-1.5">
